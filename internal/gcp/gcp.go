...
func (g *GCP) makeServiceAccount(ctx context.Context, data *executor.Data) (*adminpb.ServiceAccount, error) {
	...
	serviceAccountJSONPath := input.Recv(
		...
		func(s string) error {
			...
			if !stat.IsDir() {
				return errors.New("Path already exists, but it is not a directory")
			}
	...
	serviceAccountJSONPath, err = expandTildePath(serviceAccountJSONPath)
...
func expandTildePath(s string) (string, error) {
	...
	if err != nil {
		return s, errors.Wrap(err, "Failed expanding path containing '~'")
	}
