...
func fillDoData(ctx context.Context, storageClient *storage.Client, doData *DoData) {
	...
	doData.Bucket = uuid.New().String()

	fmt.Println("- We have autogenerated the bucket name:", doData.Bucket)

	inputBucketName := userInput(
		...
		func(bucketName string) error {
			...
			if bucketName == "" {
				return nil // Use autogenerated bucket name
			}
